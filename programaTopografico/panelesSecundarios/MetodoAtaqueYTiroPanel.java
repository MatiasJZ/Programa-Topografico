...
public class MetodoAtaqueYTiroPanel extends JPanel {
    ...
    private final Icon iconoRB = new RadioButtonCustom(22);
    ...
    private JPanel crearReglaje() {
        ...
        JPanel panel = FabricaComponentes.crearBlindado("REGLAJE");
        ...
        rbDisparosReglaje = FabricaComponentes.crearRadio("DISPAROS", iconoRB);
        rbRafagaReglaje  = FabricaComponentes.crearRadio("RÁFAGA", iconoRB);
        FabricaComponentes.agrupar(rbDisparosReglaje, rbRafagaReglaje);

        // MODO DE FUEGO
        JPanel modoPanel = FabricaComponentes.crearFila();
        modoPanel.add(rbDisparosReglaje);
        modoPanel.add(rbRafagaReglaje);
        grid.add(FabricaComponentes.crearItem("MODO DE FUEGO:", modoPanel));
        ...
        grid.add(FabricaComponentes.crearItem("VOLUMEN:", volumenReglaje));

        // ESPOLETA
        JComboBox<String> comboEspoletaReglaje =
        		FabricaComponentes.crearCombo(new String[]{"I","VT","CM","DELAY"}, comboSize);

        comboEspoletaReglaje.addActionListener(e ->
            comboEspoleta.setSelectedItem(comboEspoletaReglaje.getSelectedItem())
        );

        grid.add(FabricaComponentes.crearItem("ESPOLETA:", comboEspoletaReglaje));

        // GRANADA
        JComboBox<String> comboGranadaReglaje =
        		FabricaComponentes.crearCombo(new String[]{"HE","IL","WP","HUMO"}, comboSize);

        comboGranadaReglaje.addActionListener(e ->
            comboGranada.setSelectedItem(comboGranadaReglaje.getSelectedItem())
        );

        grid.add(FabricaComponentes.crearItem("GRANADA:", comboGranadaReglaje));
    ...
    private JPanel crearPanelMisionDeFuego() {
        ...
        rbEficacia = FabricaComponentes.crearRadio("EFICACIA", iconoRB);
        rbReglare = FabricaComponentes.crearRadio("REGLARE", iconoRB);
        rbSupresion = FabricaComponentes.crearRadio("SUPRESIÓN", iconoRB);
        rbSupresionInmediata = FabricaComponentes.crearRadio("SUPRESIÓN INMEDIATA", iconoRB);
    ...
    private JPanel crearSelectorMetodoTiro() {
        ...
        rbMetodo = FabricaComponentes.crearRadio("MÉTODO DE ATAQUE", iconoRB);
        rbTiro = FabricaComponentes.crearRadio("TIRO Y CONTROL", iconoRB);
        rbReglajeSelector = FabricaComponentes.crearRadio("REGLAJE", iconoRB);
    ...
    private JPanel crearMetodoAtaque() {
        JPanel panel = FabricaComponentes.crearBlindado("MÉTODO DE ATAQUE");
        ...
        comboEfectoDeseado = FabricaComponentes.crearCombo(efectos, comboSize);
        grid.add(FabricaComponentes.crearItem("EFECTO DESEADO:", comboEfectoDeseado));
        
        rbDisparosAtaque = FabricaComponentes.crearRadio("DISPAROS", iconoRB);
        rbRafagaAtaque = FabricaComponentes.crearRadio("RÁFAGA", iconoRB);
        FabricaComponentes.agrupar(rbDisparosAtaque, rbRafagaAtaque);
        
        JPanel modoPanel = FabricaComponentes.crearFila();
        modoPanel.add(rbDisparosAtaque);
        modoPanel.add(rbRafagaAtaque);
        grid.add(FabricaComponentes.crearItem("MODO:", modoPanel)); 

        // CERCANO 
        grid.add(FabricaComponentes.crearGrupoRadio("CERCANO:", rb -> {
            rbCercanoSi = FabricaComponentes.crearRadio("Sí", iconoRB);
            rbCercanoNo = FabricaComponentes.crearRadio("No", iconoRB);
            FabricaComponentes.agrupar(rbCercanoSi, rbCercanoNo);
            rb.add(rbCercanoSi);
            rb.add(rbCercanoNo);
        }));

        // GRAN ÁNGULO
        grid.add(FabricaComponentes.crearGrupoRadio("GRAN ÁNGULO:", rb -> {
            rbGranAnguloSi = FabricaComponentes.crearRadio("Sí", iconoRB);
            rbGranAnguloNo = FabricaComponentes.crearRadio("No", iconoRB);
            FabricaComponentes.agrupar(rbGranAnguloSi, rbGranAnguloNo);
            rb.add(rbGranAnguloSi);
            rb.add(rbGranAnguloNo);
        }));

        // GRANADA 
        comboGranada = FabricaComponentes.crearCombo(new String[]{"HE","IL","WP", "HUMO"}, comboSize);
        grid.add(FabricaComponentes.crearItem("GRANADA:", comboGranada));

        // ESPOLETA
        comboEspoleta = FabricaComponentes.crearCombo(new String[]{"I","VT","CM", "DELAY"}, comboSize);
        grid.add(FabricaComponentes.crearItem("ESPOLETA:", comboEspoleta));
        ...
        grid.add(FabricaComponentes.crearItem("VOLUMEN:", txtVolumen));

        comboHaz = FabricaComponentes.crearCombo(new String[]{"PARALELO","CONVERGENTE","ABIERTO","ESPECIAL","CIRCULAR","E - LINEAL","E - CIRCULAR",
        		"E - RECTANGULAR","E - IRREGULAR","-"}, comboSize);
        grid.add(FabricaComponentes.crearItem("HAZ:", comboHaz));
    ...
    private JPanel crearTiroYControl() {
        ...
        JPanel panel = FabricaComponentes.crearBlindado("TIRO Y CONTROL");
        JPanel grid = new JPanel(new GridLayout(4, 2, 15, 12));
        grid.setBackground(Color.BLACK);
        Dimension comboSize = new Dimension(220, 30);

        comboPiezas = FabricaComponentes.crearCombo(new String[]{"1","2","3","4","5","6"}, comboSize);
        grid.add(FabricaComponentes.crearItem("PIEZAS:", comboPiezas));

        comboSeccion = FabricaComponentes.crearCombo(new String[]{"IZQUIERDA","DERECHA"}, comboSize);
        grid.add(FabricaComponentes.crearItem("SECCIÓN:", comboSeccion));

        // FGO CONTINUO
        grid.add(FabricaComponentes.crearGrupoRadio("FGO CONT:", rb -> {
            rbFgoSi = FabricaComponentes.crearRadio("Sí", iconoRB);
            rbFgoNo = FabricaComponentes.crearRadio("No", iconoRB);
            FabricaComponentes.agrupar(rbFgoSi, rbFgoNo);
            rb.add(rbFgoSi);
            rb.add(rbFgoNo);
        }));

        // TES
        grid.add(FabricaComponentes.crearGrupoRadio("TES:", rb -> {
            rbTesSi = FabricaComponentes.crearRadio("Sí", iconoRB);
            rbTesNo = FabricaComponentes.crearRadio("No", iconoRB);
            FabricaComponentes.agrupar(rbTesSi, rbTesNo);
            rb.add(rbTesSi);
            rb.add(rbTesNo);
        }));
    ...
    private JPanel crearBotonEnviar() {
        ...
        rbCuandoListo = FabricaComponentes.crearRadio("CUANDO LISTO", iconoRB);
        rbAMiOrden = FabricaComponentes.crearRadio("A MI ORDEN", iconoRB);
        FabricaComponentes.agrupar(rbCuandoListo, rbAMiOrden);
